...
module.exports = function (opts) {
  ...
  function processChanges (start) {
    ...
    pump(
      feed.createReadStream({start: start, live: live}),
  ...
  function printProgress () {
    ...
    if (live) {
      diff.write(
        util.format('downloaded %d [%s/s]\n', state.downloaded, prettierBytes(state.bps)) +
        util.format('processed  %d [%s/sec]', processedCounter, state.pps.toFixed(1))
      )
    } else {
