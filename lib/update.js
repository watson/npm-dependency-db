...
module.exports = function (db, opts) {
  ...
  updater
    ...
    .on('running', function () {
      lastFeedLength = updater.feed.blocks
      remaining = updater.feed.blocksRemaining()
    })
  ...
  function trackSyncing () {
    updater.feed.on('download', function (block, data) {
      ...
      downloaded++
      if (updater.feed.blocks !== lastFeedLength) {
        remaining += updater.feed.blocks - lastFeedLength
        lastFeedLength = updater.feed.blocks
  ...
  function printProgress () {
    ...
    if (updater.live) {
      ...
      var totalToProcess = updater.feed.blocks - updater.startBlock
